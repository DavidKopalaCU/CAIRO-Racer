...
from webots_ros2_utils.srv import SpawnUrdfRobot
...
class Ros2Supervisor(Node):
    def __init__(self):
        ...
        self.get_logger().warn('ROS 2 SUPERVISOR START')

        self.__robot = Supervisor()
        self.__timestep = int(self.__robot.getBasicTimeStep())

        # /clock topic
        self.create_timer(1 / 1000, self.supervisor_step_callback)
        ...
        srv = self.create_service(SpawnUrdfRobot, 'spawn_urdf_robot', self.spawn_urdf_robot_callback)
        # self.create_subscription(String, 'remove_urdf_robot', self.__remove_urdf_robot_callback, qos_profile_services_default)

    def spawn_urdf_robot_callback(self, request, response):
        self.get_logger().info('ROS 2 SUPERVISOR SERVICE REQUEST')
        ...
        if robot_name == '':
            self.get_logger().error('Ros2Supervisor cannot import an unnamed URDF robot. Please specifiy it with name="" in the URDFSpawner object.')
            response.success = False
            return response
        if robot_name in self.__urdf_robots_list:
            self.get_logger().error('The URDF robot name "' + str(robot_name) + '" is already used by another robot! Please specifiy a unique name.')
            response.success = False
            return response
        ...
        if robot.urdf_path:
            ...
            self.get_logger().error('Ros2Supervisor can not import a URDF file without a specified "urdf_path" or "robot_description" in the URDFSpawner object.')
            response.success = False
            return response
    ...
    def supervisor_step_callback(self):
